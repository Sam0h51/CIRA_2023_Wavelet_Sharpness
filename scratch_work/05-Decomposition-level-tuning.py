...
fig, axs = plt.subplots(len(decomposition_level), number_of_quantiles)
...
for i in range(number_of_quantiles):
    quantile_indices.append(i*quantile_step)

quantile_indices[number_of_quantiles-1] = 263
...
for level_index, data_list in enumerate(data_storage):
    ...
    for ax_index, quantile_index in enumerate(quantile_indices):        
        ...
        axs[level_index][ax_index].set_xticks([])
        axs[level_index][ax_index].set_yticks([])
        axs[level_index][ax_index].set_xlabel('Sharpness: {sharpness:.3f}  Blur Extent: {blur:.3f}'
                                              .format(**data_list[quantile_index]),
                                              fontsize=6)
        axs[level_index][ax_index].set_ylabel('Image {original_index}'
                                              .format(**data_list[quantile_index]),
                                              fontsize=6)
        axs[level_index][ax_index].set_title(f'{decomposition_level[level_index]}: {100*quantile_index/263:.0f}th Quantile',
                                             fontsize=10)
        axs[level_index][ax_index].imshow(data_list[quantile_index]['data'],
                        ...
                        for l in range(2):
                            if(data_list[quantile_index]['edge_maps'][0][2*i+k][2*j+l] > threshold):
                                indicator_patch = patches.Rectangle((col_lo_index + l*patch_size - .5,
                                                                    ...
                                                                    alpha=.2)
                                axs[level_index][ax_index].add_patch(indicator_patch)
