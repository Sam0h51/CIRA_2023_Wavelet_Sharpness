...
for sample in range(264):
    ...
    # Stores how many levels of wavelet decomposition are
    # desired. We also use this later to compute patch
    # sizes to indicate edge type.
    level = 5
    ...
    for i in range(shape[0]):
        for j in range(shape[1]):
            ...
            if(total_edge_indicator[i][j] > 0):
                ...
                if(edge_max[2][i][j] > edge_max[1][i][j] and edge_max[2][i][j] > edge_max[0][i][j]):
    ...
    for i in range(total_edge_indicator.shape[0]):
        for j in range(total_edge_indicator.shape[1]):
            ...
            row_hi_index = min(row_lo_index + 2**(level + 1), data.shape[0])
            col_hi_index = min(col_lo_index + 2**(level + 1), data.shape[1])

            if(sharp_edge_indicator[i][j] > 0.5):
                indicator_patch = patches.Rectangle((col_lo_index, row_lo_index),
                                                    2**(level+1), 2**(level+1),
